...
const validateTransaction = ({ type, category, amount, description, date }) => {
  ...
  if (type === 'expense' && !expenseCategories.includes(category)) return 'Please select a valid expense category.';
  if (!Number.isFinite(Number(amount)) || Number(amount) <= 0) return 'Amount must be greater than 0.';
  if (!description?.trim()) return 'Description is required.';
  if (date && Number.isNaN(new Date(date).getTime())) return 'Enter a valid date.';
  return null;
};
...
export const createTransaction = async (request, response, next) => {
  try {
    ...
    const validationError = validateTransaction({ type, category, amount, description, date });
    if (validationError) return response.status(400).json({ success: false, message: validationError });
    const transaction = await Transaction.create({ user: request.user._id, type, category: category.trim(), amount: Number(amount), description: description.trim(), date });
...
export const updateTransaction = async (request, response, next) => {
  try {
    const existingTransaction = await Transaction.findOne({ _id: request.params.id, user: request.user._id });
    if (!existingTransaction) return response.status(404).json({ success: false, message: 'Transaction not found.' });
    const updatedValues = { ...existingTransaction.toObject(), ...request.body };
    const validationError = validateTransaction(updatedValues);
    if (validationError) return response.status(400).json({ success: false, message: validationError });
    const transaction = await Transaction.findByIdAndUpdate(request.params.id, { ...request.body, category: updatedValues.category.trim(), description: updatedValues.description.trim(), amount: Number(updatedValues.amount) }, { new: true, runValidators: true });
